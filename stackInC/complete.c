...
int precedence(char c)
{
    if (c == '^')
    {
        return 3; // Giving highest precedence
    }
    else if (c == '*' || c == '/')
    {
        return 2;
...
void infix_to_postfix(char q[])
{
    ...
    for (int i = 0; i <= len; i++)
    {
        ...
        // Case 2: If it is an opening bracket, push it to the stack
        else if (q[i] == '(')
        {
            push(st, q[i]);
        }
        // Case 3: If it is a closing bracket, repeatedly add elements to the answer and pop from the stack until its opening bracket is found
        else if (q[i] == ')')
        {
